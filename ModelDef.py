...
# computes the travel time in minutes from p to q
# based on euclidean dist
# for now, random variable to determine if transit or pfh is faster
...
def computeAgg(p, q, mode, timeWeight, costWeight):
    ...
    if timeWeight > 0:
        if costWeight > 0:
            return ((time * timeWeight) + (cost * costWeight)) # time > 0, cost > 0 - care about both
        else:
            return (time * timeWeight) # time > 0, cost = 0 - dont care about cost
    else:
        if costWeight > 0:
            return (cost * costWeight) # time = 0, cost > 0 - dont care about time
        else:
            return 0 # time = 0, cost = 0 - dont care about anything
        


def computePercent(model):
...
class TransModel(Model):
    ...
    def __init__(self, N, width, height):
        ...
        self.datacollector = DataCollector(
                model_reporters = {'Percent': computePercent})
...
class TransAgent(Agent):
    ...
    def pickMode(self, p, q):
        transit = computeAgg(p, q, 1, 0.2, 0.5)
        pfh = computeAgg(p, q, 2, 0.2, 0.5)
    ...
    def move(self):
        ...
        # pick transportation method and move
